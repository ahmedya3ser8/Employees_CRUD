...
const AddEmployee = ({closeAddModal}: {closeAddModal: () => void}) => {
  const { register, handleSubmit, errors, submitForm } = useAddEmployee({closeAddModal})
  return (
    <div className="add-employee-modal fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 min-h-screen">
      <div className="bg-white rounded-2xl shadow-xl max-w-md w-full">
        <div className="modal_header flex justify-between items-center mb-4 border-b border-gray-300 p-5">
          <h2 className="text-xl font-semibold">Add Employee</h2>
          <button onClick={() => closeAddModal()} className="text-gray-500 text-3xl hover:text-gray-700">
            &times;
          </button>
        </div>
        <form className="modal_body p-5" onSubmit={handleSubmit(submitForm)} >
          ...
          <div className="modal_footer p-5 flex gap-4 justify-end bg-gray-200">
            <button onClick={() => closeAddModal()} type="button">Cancel</button>
            <button type="submit" className="py-2 px-4 bg-[#218838] text-white rounded-md">Add</button>
          </div>
        </form>
      </div>
    </div>
